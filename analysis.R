...
bet_t_plot <- ggplot(data_for_lmer, aes(x=date,y=R0,group=state,col=state)) + geom_line() + facet_wrap(~state) + theme_bw() + ylab("Beta(t)") + xlab("Date")
ggsave("/Users/gcgibson/mech_bayes_paper/beta_t_plot.png",bet_t_plot,device="png",width=8,height=4)

det_probs <- matrix(NA,nrow=137,ncol=length(states_to_plot))
counter <- 1
for (file in states_to_plot){
...
detection_plot <- ggplot(data_for_detection, aes(x=date,y=R0,group=state,col=state)) + geom_line() + facet_wrap(~state) + 
