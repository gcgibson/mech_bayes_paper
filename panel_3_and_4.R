...
#### IN SAMPLE
ny_in_sample <- ny_npz$f$mcmc_samples[[1]]$beta
ca_in_sample <- ca_npz$f$mcmc_samples[[1]]$beta
fl_in_sample <- fl_npz$f$mcmc_samples[[1]]$beta
tx_in_sample <- tx_npz$f$mcmc_samples[[1]]$beta
...
#### OUT SAMPLE
ny_out_sample <- ny_npz$f$forecast_samples[[1]]$beta
ca_out_sample <- ca_npz$f$forecast_samples[[1]]$beta
fl_out_sample <- fl_npz$f$forecast_samples[[1]]$beta
tx_out_sample <- tx_npz$f$forecast_samples[[1]]$beta
...
beta_t <- ggplot() + geom_ribbon(data=tmp_mid,aes(x=date,ymax=q_u,ymin=q_l,fill="In Sample"),alpha=.5) +
  ...
  facet_wrap(~region,scales='free',nrow=1) + theme_bw() + ylab(expression(beta[t]))
